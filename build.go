...
This flag is not case-sensitive.`, strings.Join(orderedSteps, ", ")))
	...
	if steps["build"] {
		...
		dbg.Println("[INFO] Building Capybara...")
		...
		if output, err = cmd.CombinedOutput(); err != nil {
			dbg.Printf("[ERROR] Error building capybara: %s\n%s\n",
				err.Error(),
				output)
			os.Exit(1)
		}
...
func worker(n int, op string, pkgq <-chan string, errq chan<- error, wg *sync.WaitGroup) {
	...
	for folder := range pkgq {
		pkg = "github.com/blicero/capybara/" + folder
		...
		case "nilaway":
			cmd = exec.Command(op, "-include-pkgs=github.com/blicero/capybara", pkg)
		default:
